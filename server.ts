...
تلاش و سلیقه شما در طراحی این دوره بی‌نظیر است. خسته نباشید! 🎓✨`;
  ...
  app.post("/api/ai/review", async (req, res) => {
    try {
      ...
      if (!activeKey) {
        console.warn("GEMINI_API_KEY is missing or invalid. Using smart simulated AI reviewer.");
        const simulated = getSimulatedAiReview(lesson, studentName, answers);
...
-------------------------`;
        ...
        const prompt = `
تو دلسوزترین، مهربان‌ترین و متبحرترین مربی و استادیار ارشد برنامه‌نویسی و طراحی رابط کاربری (UI) هستی. وظیفه تو بررسی پاسخ‌های ارسالی دانش‌آموز به نام "${studentName}" برای درس "${lesson.title}" است.

محتوای درس:
${lesson.content}

پاسخ‌های ثبت‌شده دانش‌آموز:
${formattedAnswers}

دستورالعمل‌های بررسی و نمره‌دهی:
1. بررسی مهربانانه و عادلانه: پاسخ‌های هر سوال را بر اساس اصول طراحی مدرن و مفاهیم درس بسنج. حداکثر امتیاز کل ${maxPoints} است. نمره نهایی را متناسب با تلاش و کیفیت پاسخ‌ها مشخص کن.
2. اعداد انگلیسی: حتماً در پاسخ خود از اعداد انگلیسی (مانند 1، 2، 3, 10, 85, 100) استفاده کن و اصلاً از اعداد فارسی استفاده نکن.
3. تفکیک کامل و هدفمند بین بازخورد مربی و دانش‌آموز:
   الف) برای مربی (teacherFeedback):
       - لحن: حرفه‌ای، تخصصی، فنی، تحلیلی و آموزشی.
       - محتوا: ارزیابی موشکافانه تک‌تک سوالات با بیان نقاط ضعف و قوت دقیق در طراحی یا کدهای هنرجو. ارائه دلایل فنی برای کسر نمره، مشخص کردن بخش‌هایی از کد که درست کار نکرده‌اند و ارائه پیشنهاد به مدرس برای چگونگی تدریس یا ارائه نظر نهایی به دانش‌آموز. مدرس از این بازخورد برای هدایت و ارزشیابی نهایی استفاده می‌کند.
   ب) برای دانش‌آموز (studentFeedback):
       - لحن: فوق‌العاده صمیمی، همدلانه، انگیزه‌بخش و حامی (به عنوان مربی همراه دلسوز).
       - محتوا: دانش‌آموز را عمیقاً تشویق کن و به تلاشی که کرده ارج بنه. برای مواردی که نیاز به اصلاح دارند، به هیچ وجه پاسخ نهایی یا کدهای اصلاح‌شده را لو نده و مستقیم ننویس. در عوض با طرح سوالات تفکر برانگیز و دادن سرنخ‌های طلایی، او را راهنمایی کن تا خودش ترغیب شود اشکالات را بیابد. صراحتاً مشخص کن که به کدام بخش از **متن درس** (مثل بخش هماهنگی کلاس‌ها، توازن، افکت‌های هاور یا ترنزیشن دکمه) بازگردد و مجدداً آن نکات را بخواند تا خودش پاسخ را تکمیل و نمره‌اش را ارتقا دهد.

خروجی نهایی باید قالب JSON با سه کلید زیر باشد:
- grade: نمره نهایی کلی دانش‌آموز به عنوان یک عدد صحیح (مثلاً 85)
- teacherFeedback: بازخورد تحلیلی و فنی مخصوص مدرس (با فرمت Markdown)
- studentFeedback: راهنما و بازخورد انگیزشی و آموزشی همراه با سرنخ مخصوص خود دانش‌آموز برای خودآموزی و ارتقای پاسخ (با فرمت Markdown)
`;
        ...
        console.error("Real Gemini Review failed, falling back to simulator:", innerErr);
        const simulated = getSimulatedAiReview(lesson, studentName, answers);
        return res.json(simulated);
      }
    } catch (error: any) {
